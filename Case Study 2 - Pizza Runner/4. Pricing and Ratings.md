...
- In this question we will use 2 tables namely runner orders and price tables.
- Because there are some known data issues with runner orders table, we need to fix it first.
- Step 1 : Fixing customer_orders and runner_orders table
```sql
...
CREATE TEMP TABLE runner_orders_cleaned AS
	WITH first_layer AS(
		SELECT order_id,
				runner_id,
			CAST(CASE WHEN pickup_time = '' THEN NULL
...
```
--
- Step 3 : how much money has Pizza Runner made so far if there are no delivery fees?
```sql
WITH order_price AS(	
	SELECT t1.order_id,
			t1.pizza_id,
			COUNT(t1.pizza_id) * t3.pizza_price as pizza_revenue
	...
	GROUP BY t1.order_id, t1.pizza_id, t3.pizza_price
	ORDER BY 1
)
	SELECT SUM(pizza_revenue)as revenue
	FROM order_price;
```  
	
So we get the revenue Pizza Runner made, which no delivery fees, were $138
--
