...
- Create `Subquery` and Use `UNNEST()` to separate values in toppings column as `topping_id`
...
### Steps:
- Use `JOIN` to merge and return only rows that have matching values in both tables
- Create `Subquery` and Use `UNNEST()` to separate values in extras column as `topping_id`
- Use `CAST()` to convert datatype into INTEGER
...
### Steps:
- Use `JOIN` to merge and return only rows that have matching values in both tables
- Create `Subquery` and Use `UNNEST()` to separate values in exclusions column as `topping_id`
- Use `CAST()` to convert datatype into INTEGER
